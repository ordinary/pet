...
public interface CommodityDAO {
	...
	String INSERT_SQL = "cid ,click_url,commission ,commission_num ,"
	...
	@SQL("insert into "
			...
			+ "("
			+ INSERT_SQL
			+ " ) values (:1.cid,:1.clickUrl,:1.commission"
	...
	@SQL("select " + SELECT_SQL + " from " + TABLE
			+ "where cid =:1 order by rand(:3)   limit :2 ,:3  ")
	public List<Commodity> queryByCid(int cid, int startId, int count);
	
	@SQL("select " + SELECT_SQL + " from " + TABLE
			+ "where  #if((:1!=0)){cid =:1} order by seller_credit_score desc  limit :2 ,:3  ")
	public List<Commodity> queryCmsByCid(int cid, int startId, int count);

	@SQL("select " + SELECT_SQL + " from " + TABLE + "where num_iid =:1")
	public Commodity queryByNumIid(long numIid);

	@SQL("select " + SELECT_SQL + " from " + TABLE
			+ "where num_iid >:1 order by num_iid  limit :2")
	public List<Commodity> queryCommodityByNumIidOrderbyNumIid(int startNumIid,
			int count);
