...
public class JadeDataSourceFactory implements DataSourceFactory {
    ...
    public JadeDataSourceFactory(){
    	instanceCount++;
    	if(instanceCount > 3){
    		System.err.println("ERROR ERROR ERROR: too many NewDataSourceFactory, count:" + instanceCount);
    	}
    }
	...
	private void createJadeDatasource(String catalog) {
		...
		BasicDataSource masterBasicDatasource = initBasicDatasource(catalog, true);
		masterDataSource.addDatasource(masterBasicDatasource);
		
		BasicDataSource slaveBasicDatasource = initBasicDatasource(catalog, false);
	...
	private BasicDataSource initBasicDatasource(String bizName, boolean write) {
		...
		if(notfound){
			System.err.println(" ~~~~~~~~~~~~~~~Config ERROR: Not Found DB Configuration of " + bizName + ", Please tell Administrator");
		}
		...
		if("mysql".equalsIgnoreCase(dsConfig.getType())){
			...
			ds.setUrl(String.format("jdbc:mysql://%s:%s/%s?characterEncoding=utf-8&connectTimeout=100", dsConfig.getHost(), dsConfig.getPort(), dsConfig.getDatabase()));
		}
		else if("postgresql".equalsIgnoreCase(dsConfig.getType())){
			ds.setDriverClassName("org.postgresql.Driver");
			ds.setUrl(String.format("jdbc:postgresql://%s:%s/%s?connectTimeout=100", dsConfig.getHost(), dsConfig.getPort(), dsConfig.getDatabase()));
		}else{
    ...
    private JadeDataSourceConfig getDsConfig(String bizName, boolean write) {
		...
		ConfigItem dbConfig = Configurations.getInstance().getConfigById(bizName);
    	if(dbConfig == null){
    		logger.error(bizName + " DatasourceConfig not found ");
    	}else {
    		dsConfig = write ? dbConfig.getMasterDBConfig() : dbConfig.getSlaverDBConfig();
    	}
...
//        String catalog = metaData.getClass().getAnnotation(DAO.class).catalog();
        String catalog = metaData.getDAOMetaData().getDAOClass().getAnnotation(DAO.class).catalog();
        String bizName = metaData.getSQLType() == SQLType.READ ? catalog + "_R" : catalog + "_W";
        ...
        while((dataSource=cachedDsMap.get(bizName)) == null && count++ < 5){
            ...
            System.out.println( "try...to get " + bizName + "||" + count + "||" + cachedDsMap.size() );
        }
        return dataSource;
    }
}
