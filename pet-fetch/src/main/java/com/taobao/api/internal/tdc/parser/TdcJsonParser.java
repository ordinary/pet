	...
	public TdcResponse parse(String rsp) throws ApiException {
		...
		if (rootObj instanceof Map<?, ?>) {
			...
			if (rootJson.containsKey(Constants.ERROR_RESPONSE)) {
				jxRsp.setErrorCode((String)rootJson.get(Constants.ERROR_CODE));
				jxRsp.setMsg((String)rootJson.get(Constants.ERROR_MSG));
				jxRsp.setSubCode((String)rootJson.get(Constants.ERROR_SUB_CODE));
				jxRsp.setSubMsg((String)rootJson.get(Constants.ERROR_SUB_MSG));
			} else {
